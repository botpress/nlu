...
export const initDic: typeof sdk.tables.initDic = <T>(props: string[], valueGenerator: () => T): _.Dictionary<T> => {
  const values: T[] = new Array<number>(props.length).fill(0).map(valueGenerator)
