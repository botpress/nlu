...
export const listFiles: typeof sdk.datasets.listFiles = async () => {
  ...
  return Bluebird.map(types, async (type) => {
    ...
    const matchingFiles = allFiles.filter((f) => !!fileNameFormat.exec(path.basename(f)))
    ...
    return matchingFiles.map((f) => {
      ...
      const namespace = _.xor(...[fPath, basePathForType].map(splitPath)).join(NAMESPACE_SEP)
...
const _readFile = async <T extends sdk.ProblemType>(info: sdk.FileDef<T, sdk.FileType>) => {
  const { lang, name, type, namespace } = info
  const fName = _makeFileName(name, lang, info.fileType === 'dataset' ? 'ds' : 'doc')
  const fPath = path.join(__dirname, ROOT_DIR, type, namespace.split(NAMESPACE_SEP).join(path.sep) ?? '', fName)
  const fileContent = await fse.readFile(fPath, 'utf8')
  return JSON.parse(fileContent)
}

export const readDataset: typeof sdk.datasets.readDataset = <T extends sdk.ProblemType>(
  info: sdk.FileDef<T, 'dataset'>
) => {
  return _readFile(info)
}

export const readDocument: typeof sdk.datasets.readDocument = <T extends sdk.ProblemType>(
