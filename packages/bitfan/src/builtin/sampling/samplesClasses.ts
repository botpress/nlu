...
>(
  ...
  if (nClass < 0) {
    throw new Error(
      `Can't subsample a negative ammount of classes (${nClass}).`
    );
  }
  if (nClass > allClasses.length) {
    throw new Error(
      `Can't subsample ${nClass} classes out of ${allClasses.length} class.`
    );
  }
  ...
  const keepSample = (s: sdk.Sample<T>) => {
    return (
      (options.keepOOS && isOOS(s.label)) || chosenClasses.includes(s.label)
    );
  };
