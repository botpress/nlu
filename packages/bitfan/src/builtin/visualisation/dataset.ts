...
export const showDatasetsSummary: typeof visualisation.showDatasetsSummary = (...datasets: DataSet<ProblemType>[]) => {
  ...
  for (const ds of datasets) {
    ...
    const amountOfSamplesPerClass = classes.map((c) => ds.samples.filter((r) => areSame(r.label, c)).length)
    const avgSamplesPerClass = _.sum(amountOfSamplesPerClass) / amountOfSamplesPerClass.length
...
const _getClassDistributionForOneSet = <T extends ProblemType>(dataset: DataSet<T>) => {
  ...
  for (const label of allLabels) {
    distribution[makeKey(label)] = samples.filter((r) => areSame(label, r.label)).length / N
  }
